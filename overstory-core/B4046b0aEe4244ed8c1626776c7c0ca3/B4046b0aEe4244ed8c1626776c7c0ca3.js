import {
  Sprite,
  Trigger,
  Watcher,
  Costume,
  Color,
  Sound,
} from "https://unpkg.com/leopard@^1/dist/index.esm.js";

export default class B4046b0aEe4244ed8c1626776c7c0ca3 extends Sprite {
  constructor(...args) {
    super(...args);

    this.costumes = [
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA2",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA2.png",
        { x: 40, y: 10 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA22",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA22.png",
        { x: 118, y: 72 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA23",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA23.png",
        { x: 135, y: 166 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA24",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA24.png",
        { x: 142, y: 231 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA25",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA25.png",
        { x: 142, y: 246 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA26",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA26.png",
        { x: 137, y: 242 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA27",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA27.png",
        { x: 137, y: 249 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA28",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA28.png",
        { x: 147, y: 249 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA29",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA29.png",
        { x: 147, y: 249 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA210",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA210.png",
        { x: 147, y: 249 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA211",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA211.png",
        { x: 148, y: 249 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA212",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA212.png",
        { x: 143, y: 228 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA213",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA213.png",
        { x: 148, y: 237 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA214",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA214.png",
        { x: 138, y: 242 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA215",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA215.png",
        { x: 142, y: 248 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA216",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA216.png",
        { x: 123, y: 249 }
      ),
      new Costume(
        "B4046B0A-EE42-44ED-8C16-26776C7C0CA217",
        "./costumes/B4046B0A-EE42-44ED-8C16-26776C7C0CA217.png",
        { x: 118, y: 249 }
      ),
      new Costume(
        "costume1",
        "./costumes/costume1.png",
        { x: 27, y: 68 }
      ),
    ];

    this.sounds = [
      new Sound(
        "deltarune-explosion 2",
        "./sounds/deltarune-explosion 2.mp3"
      ),
      new Sound(
        "Vs Amber",
        "./sounds/Vs Amber.wav"
      ),
    ];

    this.triggers = [
      new Trigger(Trigger.GREEN_FLAG, this.whenGreenFlagClicked),
      new Trigger(
        Trigger.BROADCAST,
        { name: "Ultra explode" },
        this.whenIReceiveUltraExplode
      ),
      new Trigger(Trigger.CLONE_START, this.startAsClone),
      new Trigger(Trigger.BROADCAST, { name: "Bombs" }, this.whenIReceiveBombs),
      new Trigger(
        Trigger.BROADCAST,
        { name: "Amber fight" },
        this.whenIReceiveAmberFight
      ),
      new Trigger(
        Trigger.BROADCAST,
        { name: "Ur purple now" },
        this.whenIReceiveUrPurpleNow
      ),
      new Trigger(
        Trigger.BROADCAST,
        { name: "Boms 2" },
        this.whenIReceiveBoms2
      ),
      new Trigger(Trigger.CLONE_START, this.startAsClone2),
      new Trigger(
        Trigger.BROADCAST,
        { name: "Amber tired" },
        this.whenIReceiveAmberTired
      ),
    ];

    this.audioEffects.volume = 0;
  }

  *whenGreenFlagClicked() {
    this.audioEffects.volume = 0;
    this.visible = false;
  }

  *whenIReceiveUltraExplode() {
    while (true) {
      this.broadcast("Explode");
      yield;
    }
  }

  *startAsClone() {
    this.visible = true;
    this.costume = "costume1";
    this.size = 100;
    this.goto(this.random(-88, 92), 91);
    for (let i = 0; i < this.random(5, 40); i++) {
      this.direction += 20;
      this.y -= 5;
      yield;
    }
    yield* this.startSound("deltarune-explosion 2");
    this.size = 20;
    this.moveAhead();
    this.costume = "B4046B0A-EE42-44ED-8C16-26776C7C0CA2";
    for (let i = 0; i < 16; i++) {
      this.costumeNumber++;
      yield* this.wait(0);
      yield* this.wait(0);
      yield;
    }
    this.deleteThisClone();
  }

  *whenIReceiveBombs() {
    for (let i = 0; i < 5; i++) {
      this.createClone();
      yield* this.wait(0.5);
      yield;
    }
  }

  *whenIReceiveAmberFight() {
    this.audioEffects.volume = 100;
    this.audioEffects.pitch = -50;
    while (true) {
      yield* this.playSoundUntilDone("Vs Amber");
      yield;
    }
  }

  *whenIReceiveUrPurpleNow() {
    this.audioEffects.pitch = 0;
  }

  *whenIReceiveBoms2() {
    for (let i = 0; i < 10; i++) {
      this.createClone();
      yield* this.wait(0.8);
      yield;
    }
    yield* this.wait(1);
    this.broadcast("Ur turn");
  }

  *startAsClone2() {
    while (!!(this.costumeNumber === 18)) {
      yield;
    }
    while (true) {
      if (this.touching(this.sprites["Sprite20"].andClones())) {
        if (this.toNumber(this.stage.vars.iFrames) === 0) {
          this.stage.vars.hp -= 3;
          this.stage.vars.iFrames++;
          this.deleteThisClone();
        }
      }
      yield;
    }
  }

  *whenIReceiveAmberTired() {
    /* TODO: Implement stop other scripts in sprite */ null;
  }
}
